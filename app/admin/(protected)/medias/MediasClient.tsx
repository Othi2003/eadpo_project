...
export default function MediasAdminClient({ dossiers: initial }: { dossiers: DossierComplet[] }) {
  ...
  const DossierRow = ({
    ...
    d: DossierComplet | DossierComplet["sousDossiers"][0] | DossierComplet["sousDossiers"][0]["sousDossiers"][0]
    niveau?: number
  }) => {
    ...
    return (
      <div>
        <div style={{
          ...
          <button
            onClick={() => toggleOuvert(d.id)}
            style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: "0.5rem", flex: 1, padding: 0, fontFamily: "'Inter', sans-serif", textAlign: "left" as const }}
          >
            ...
            <div style={{ width: "32px", height: "32px", borderRadius: "8px", background: estCulte ? "rgba(124,58,237,0.08)" : "rgba(21,101,192,0.08)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              {ouvert
                ? <FolderOpen size={16} color={estCulte ? "#7C3AED" : "#1565C0"} />
                : <Folder size={16} color={estCulte ? "#7C3AED" : "#1565C0"} />
              }
            </div>
            <div>
              <p style={{ fontWeight: 700, color: "#1e3a5f", fontSize: "0.9rem", margin: 0 }}>{d.nom}</p>
              <p style={{ fontSize: "0.72rem", color: "#9CA3AF", margin: "0.1rem 0 0" }}>
                ...
                {estCulte && (
                  <span style={{ marginLeft: "0.5rem", background: "rgba(124,58,237,0.1)", color: "#7C3AED", padding: "0.1rem 0.4rem", borderRadius: "9999px", fontSize: "0.65rem", fontWeight: 700 }}>
                    CULTE
                  </span>
                )}
              </p>
          ...
          {!estCulte && (
            <div style={{ display: "flex", gap: "0.4rem" }}>
              <button
                onClick={() => { setParentId(d.id); setNomDossier(""); setModalDossier("sous-dossier") }}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 1fr))", gap: "0.5rem", marginBottom: "0.75rem" }}>
                {d.medias.map(f => (
                  <div key={f.id} style={{ position: "relative", borderRadius: "0.75rem", overflow: "hidden", background: "#f5f9ff", border: "1px solid rgba(21,101,192,0.08)" }}>
                    ...
                    <button
                      onClick={() => suppFichier(f.id)}
                      style={{ position: "absolute", top: "4px", right: "4px", background: "rgba(0,0,0,0.5)", border: "none", borderRadius: "9999px", width: "20px", height: "20px", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}
                    >
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{
        ...
        <div>
          <h1 style={{ fontSize: "1.5rem", fontWeight: 800, color: "#1e3a5f", margin: 0 }}>Médias</h1>
          <p style={{ color: "#9CA3AF", fontSize: "0.85rem", margin: "0.15rem 0 0" }}>
            {dossiers.length} dossier{dossiers.length > 1 ? "s" : ""}
          </p>
        </div>
        <button
          onClick={() => { setParentId(null); setNomDossier(""); setModalDossier("creer") }}
          style={{
      ...
      >
        {dossiers.length === 0 ? (
          <p style={{ textAlign: "center", color: "#9CA3AF", padding: "3rem" }}>
            Aucun dossier. Créez-en un ou ajoutez des cultes.
          </p>
        ) : (
      ...
      <Modal
        ...
        title={
          modalDossier === "creer" ? "Nouveau dossier"
          : modalDossier === "sous-dossier" ? "Nouveau sous-dossier"
          : "Renommer le dossier"
        }
      >
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          <button
            onClick={soumettresDossier}
            disabled={loading || !nomDossier.trim()}
            style={{
              padding: "0.875rem", background: "linear-gradient(135deg, #1565C0, #1E88E5)",
              color: "white", border: "none", borderRadius: "0.875rem",
              fontWeight: 700, fontSize: "0.95rem", cursor: "pointer",
              fontFamily: "'Inter', sans-serif",
              opacity: loading || !nomDossier.trim() ? 0.7 : 1,
            }}
          >
      ...
      <Modal open={modalFichier} onClose={() => setModalFichier(false)} title="Ajouter un fichier">
        <div style={{ display: "flex", flexDirection: "column", maxHeight: "70vh", overflow: "hidden" }}>
          <div className="adm-scroll" style={{ flex: 1, overflowY: "auto", paddingRight: "4px", display: "flex", flexDirection: "column", gap: "1rem" }}>
              ...
              <div style={{ display: "flex", gap: "0.75rem" }}>
                {(["PHOTO", "VIDEO"] as const).map(t => (
                  <button
                    key={t}
                    type="button"
                    onClick={() => { setTypeFichier(t); setUrlFichier(""); setUrlsFichiers([]) }}
                    style={{
                      display: "flex", alignItems: "center", gap: "0.4rem",
                      padding: "0.6rem 1.25rem", borderRadius: "9999px",
                      border: "1.5px solid", fontSize: "0.85rem", fontWeight: 600,
                      cursor: "pointer", fontFamily: "'Inter', sans-serif",
                      background: typeFichier === t ? "#1565C0" : "white",
                      color: typeFichier === t ? "white" : "#6B7280",
                      borderColor: typeFichier === t ? "#1565C0" : "#E5E7EB",
                    }}
                  >
          ...
          {/* Bouton fixe en bas */}
          <div style={{ flexShrink: 0, paddingTop: "1.25rem", borderTop: "1px solid rgba(21,101,192,0.07)", marginTop: "0.5rem" }}>
            <button
