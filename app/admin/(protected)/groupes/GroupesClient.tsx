...
export default function GroupesAdminClient({ groupes: initial }: { groupes: GroupeComplet[] }) {
  ...
  const modifObjectif = (i: number, val: string) =>
    setForm(f => ({ ...f, objectifs: f.objectifs.map((o, idx) => idx === i ? val : o) }))
  const suppObjectif = (i: number) =>
    setForm(f => ({ ...f, objectifs: f.objectifs.filter((_, idx) => idx !== i) }))
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{
        ...
        <div>
          <h1 style={{ fontSize: "1.5rem", fontWeight: 800, color: "#1e3a5f", margin: 0 }}>Groupes</h1>
          ...
          {groupes.map(g => (
            <div key={g.id} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", padding: "1.25rem 1.5rem", gap: "1rem" }}>
                ...
                >
                  <div>
                    <p style={{ fontWeight: 700, color: "#1e3a5f", fontSize: "0.95rem", margin: 0 }}>{g.nom}</p>
                    <p style={{ fontSize: "0.78rem", color: "#9CA3AF", margin: "0.15rem 0 0" }}>
                      {g.objectifs.length} objectif{g.objectifs.length > 1 ? "s" : ""} · {g.images.length} image{g.images.length > 1 ? "s" : ""}
                    </p>
                  </div>
                  <ChevronDown
                    ...
                    style={{
                      marginLeft: "auto",
                      transform: ouvert === g.id ? "rotate(180deg)" : "rotate(0deg)",
                ...
                <div style={{ display: "flex", gap: "0.5rem", flexShrink: 0 }}>
                  <button
                    onClick={() => ouvrir("modifier", g)}
                    style={{
                      display: "flex", alignItems: "center", gap: "0.4rem",
                      padding: "0.6rem 1rem", background: "rgba(21,101,192,0.06)",
                      color: "#1565C0", border: "none", borderRadius: "0.75rem",
                  ...
                  <button
                    ...
                    style={{
                      padding: "0.6rem 0.875rem", background: "rgba(239,68,68,0.07)",
                      color: "#DC2626", border: "none", borderRadius: "0.75rem",
              ...
              {ouvert === g.id && (
                <div style={{ borderTop: "1px solid rgba(21,101,192,0.06)", padding: "1.5rem" }}>
                  {g.description && (
                    <p style={{ fontSize: "0.88rem", color: "#374151", marginBottom: "1rem", lineHeight: 1.7 }}>
                      {g.description}
                    </p>
                  )}
                  {g.images.length > 0 && (
                    <div style={{ marginBottom: "1.25rem" }}>
                      <p style={{ fontSize: "0.72rem", fontWeight: 700, color: "#9CA3AF", letterSpacing: "0.1em", textTransform: "uppercase" as const, marginBottom: "0.75rem" }}>
                        Images
                      </p>
                        ...
                        {g.objectifs.map((o, i) => (
                          <div key={o.id} style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                            <span style={{
                              ...
                              fontSize: "0.65rem", fontWeight: 800, color: "#1565C0", flexShrink: 0,
                            }}>
                              {i + 1}
                            </span>
      ...
      <Modal
        ...
        title={modal === "creer" ? "Nouveau groupe" : "Modifier le groupe"}
      >
            ...
            <div>
              ...
              {form.objectifs.length === 0 && (
                <p style={{ fontSize: "0.82rem", color: "#9CA3AF", fontStyle: "italic" }}>
                  Aucun objectif — cliquez sur « + Ajouter » pour en créer un.
                </p>
              )}
                ...
                {form.objectifs.map((o, i) => (
                  <div key={i} style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                    <input
                      ...
                      style={{
                        flex: 1, padding: "0.7rem 1rem", border: "1.5px solid #E5E7EB",
                        borderRadius: "0.75rem", fontSize: "0.88rem", outline: "none",
