...
export default function StructuresAdminClient({ structures: initial }: { structures: StructureComplete[] }) {
  ...
  const modifObjectif = (i: number, val: string) =>
    setFormStructure(f => ({ ...f, objectifs: f.objectifs.map((o, idx) => idx === i ? val : o) }))
  const suppObjectif = (i: number) =>
    setFormStructure(f => ({ ...f, objectifs: f.objectifs.filter((_, idx) => idx !== i) }))
  const ajouterImages = (urls: string[]) =>
    setFormStructure(f => ({ ...f, images: [...f.images, ...urls] }))
  const suppImage = (i: number) =>
    setFormStructure(f => ({ ...f, images: f.images.filter((_, idx) => idx !== i) }))
  ...
  const ModalForm = ({ onSubmit }: { onSubmit: () => void }) => (
      ...
      <div className="adm-scroll" style={{ flex: 1, overflowY: "auto", paddingRight: "4px", display: "flex", flexDirection: "column", gap: "1rem" }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap", alignItems: "flex-end" }}>
            ...
            <UploadButton value="" onChange={() => {}} onChangeMultiple={ajouterImages} label="" multiple />
          </div>
        </div>
      </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          {structures.map(s => (
            <div key={s.id} style={{
              ...
              border: "1px solid rgba(21,101,192,0.08)", overflow: "hidden",
            }}>
              <div style={{ display: "flex", alignItems: "center", padding: "1.25rem 1.5rem", gap: "1rem" }}>
                ...
                >
                  ...
                  <div>
                    <p style={{ fontWeight: 700, color: "#1e3a5f", fontSize: "0.95rem", margin: 0 }}>{s.sigle} — {s.nom}</p>
                    <p style={{ fontSize: "0.78rem", color: "#9CA3AF", margin: "0.15rem 0 0" }}>
                      {s.membres.length} membre{s.membres.length > 1 ? "s" : ""} · {s.objectifs.length} objectif{s.objectifs.length > 1 ? "s" : ""} · {s.images.length} image{s.images.length > 1 ? "s" : ""}
                    </p>
                  </div>
                      ...
                      <div style={{ display: "flex", flexDirection: "column" as const, gap: "0.4rem" }}>
                        {s.objectifs.map((o, i) => (
                          <div key={o.id} style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                            ...
                            <p style={{ fontSize: "0.88rem", color: "#374151", margin: 0 }}>{o.contenu}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                      ...
                      {s.membres.map(m => (
                        <div key={m.id} style={{ background: "#f5f9ff", borderRadius: "1rem", overflow: "hidden", border: "1px solid rgba(21,101,192,0.08)" }}>
                          ...
                          <div style={{ padding: "0.75rem" }}>
                            <p style={{ fontWeight: 700, color: "#1e3a5f", fontSize: "0.85rem", margin: 0 }}>{m.prenom} {m.nom}</p>
                            <p style={{ fontSize: "0.72rem", color: "#9CA3AF", margin: "0.15rem 0 0" }}>{m.titre}</p>
      ...
      <Modal open={!!modalStructure} onClose={() => setModalStructure(null)} title={modalStructure === "creer" ? "Nouvelle structure" : "Modifier la structure"}>
        <ModalForm onSubmit={soumettreStructure} />
      </Modal>
      ...
      <Modal open={!!modalMembre} onClose={() => setModalMembre(null)} title={modalMembre === "creer" ? "Ajouter un membre" : "Modifier le membre"}>
        <div style={{ display: "flex", flexDirection: "column", maxHeight: "70vh", overflow: "hidden" }}>
          ...
          <div style={{ flexShrink: 0, paddingTop: "1.25rem", borderTop: "1px solid rgba(21,101,192,0.07)", marginTop: "0.5rem" }}>
            <button onClick={soumettresMembre} disabled={loading} style={{ width: "100%", padding: "0.875rem", background: "linear-gradient(135deg, #1565C0, #1E88E5)", color: "white", border: "none", borderRadius: "0.875rem", fontWeight: 700, fontSize: "0.95rem", cursor: "pointer", fontFamily: "'Inter', sans-serif", opacity: loading ? 0.7 : 1 }}>
              {loading ? "Enregistrement..." : "Enregistrer"}
            </button>
          </div>
        </div>
